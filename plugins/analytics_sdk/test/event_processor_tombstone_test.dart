...
void main() {
  ...
  // ==================== in-flight 机制（at-most-once）测试 ====================

  test('loadCachedEvents 用 in-flight 文件过滤崩溃时正在上报的事件', () async {
    final processor = EventProcessor();
    await processor.initPersistence();

    final events = [
      {'event_id': 'inflight_1', 'event': 'page_view'},
      {'event_id': 'inflight_2', 'event': 'click'},
      {'event_id': 'safe_3', 'event': 'purchase'},
    ];
    await writeCacheLines(tempDir, events);

    // 模拟崩溃：in-flight 文件中记录了上次正在上报的两条事件
    final inFlightFile = File('${tempDir.path}/data_plus_events_inflight.txt');
    await inFlightFile.writeAsString('inflight_1\ninflight_2\n');

    await processor.loadCachedEvents();

    // in-flight 的两条被过滤，只有 safe_3 进入队列
    expect(processor.queueLength, equals(1),
        reason: 'in-flight 文件中的事件应被过滤，不重发');
  });

  test('loadCachedEvents 后 in-flight 文件被删除', () async {
    final processor = EventProcessor();
    await processor.initPersistence();
    ...
    expect(await inFlightFile.exists(), isFalse,
        reason: 'loadCachedEvents 后 in-flight 文件应被删除');
  });

  test('tombstone + in-flight 联合过滤，各自独立生效', () async {
    final processor = EventProcessor();
    await processor.initPersistence();

    final events = [
      {'event_id': 'tombstoned_1', 'event': 'page_view'},
      {'event_id': 'inflight_2', 'event': 'click'},
      {'event_id': 'recover_3', 'event': 'purchase'},
    ];
    await writeCacheLines(tempDir, events);

    // tombstone 记录已成功上报的
    final tombstone = File('${tempDir.path}/data_plus_events_tombstone.txt');
    await tombstone.writeAsString('tombstoned_1\n');

    // in-flight 记录崩溃时正在上报的
    final inFlightFile = File('${tempDir.path}/data_plus_events_inflight.txt');
    await inFlightFile.writeAsString('inflight_2\n');

    await processor.loadCachedEvents();

    // 两种机制各自过滤掉一条，只剩 recover_3
    expect(processor.queueLength, equals(1),
        reason: 'tombstone 和 in-flight 应各自独立过滤，只恢复未涉及的事件');
  });
}
