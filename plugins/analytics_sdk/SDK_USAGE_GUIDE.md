...
5. [自动埋点](#5-自动埋点)
   ...
   - [5.5 导航事件手动上报](#55-导航事件手动上报navigation)
6. [手动事件上报](#6-手动事件上报)
7. [枚举参考](#7-枚举参考)
   - [AppPageEnum（页面路由）](#apppageenum页面路由)
...
- 自动埋点：页面浏览（`app_page_view`）、全局点击（`page_click`）（均默认开启）
- 手动埋点：导航菜单点击（`navigation`）、视频/小说/漫画行为、广告、推荐等客户端事件
...
注册后 SDK 会自动追踪页面进入和离开（`app_page_view`），无需手动上报。`navigation` 事件请参见 [5.5 导航事件手动上报](#55-导航事件手动上报navigation)。
...
`AppPageEnum` 预定义的 39 个页面已自动携带 `name`，直接使用即可，**无需为它们额外调用 `addMappings`**。

`addMappings` 只在以下情况需要：业务自定义了 `AppPageEnum` 未覆盖的页面 key，且希望 SDK 内部（如路由 push 时）也能解析到正确的中文名称。

```dart
import 'package:analytics_sdk/manager/page_name_manager.dart';

// 仅为自定义页面补充映射，AppPageEnum 已有的无需重复添加
...
BottomNavigationBar(
  ...
  AnalyticsTab(AppPageEnum.discover.key, AppPageEnum.discover.name),
...
TabBar(
  ...
  AnalyticsTab(AppPageEnum.videoList.key, AppPageEnum.videoList.name),
  AnalyticsTab(AppPageEnum.novelList.key, AppPageEnum.novelList.name),
])
```
...
> ⚠️ **数据侧注意**：若之前已接入 `TabBar.withAnalytics()`，升级后服务端会新增来自 TabBar 切换的 `app_page_view` 事件，请提前告知数据侧避免指标异常。
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `key` | `String` | 是 | Tab 唯一标识，建议使用 `AppPageEnum.xxx.key`，也可传自定义字符串 |
| `name` | `String?` | 否 | Tab 展示名称；建议使用 `AppPageEnum.xxx.name`，省略时由 `PageNameMapper.getPageName(key)` 解析 |

> **注意**：`tabs` 列表顺序须与 `items` / `tabs` 保持一致，长度可以小于实际 Tab 数（多余的 Tab 会用 `tab_<index>` 兜底，不会崩溃）。

### 5.5 导航事件手动上报（navigation）

`navigation` 事件**不再**由 SDK 自动上报。仅在用户**主动点击导航菜单**（底部 Tab、顶部 Tab、侧边抽屉）时，由接入方手动触发。

> 已使用 `.withAnalytics()` 扩展的 `BottomNavigationBar` / `TabBar` 无需额外调用，扩展内部已处理。

#### trackNavigation — 上报导航点击

```dart
// 用户点击底部 Tab 时调用，应早于页面切换完成
AnalyticsSdk.instance.trackNavigation(
  pageKey: AppPageEnum.video.key,   // 'video'
  pageName: AppPageEnum.video.name, // '影视'
);
```

示例（手动管理 `BottomNavigationBar`）：

```dart
// 预定义 Tab 配置（建议用 AppPageEnum，也可用自定义字符串）
const _navTabs = [
  AppPageEnum.home,
  AppPageEnum.videoList,
  AppPageEnum.mine,
];
...
BottomNavigationBar(
  onTap: (index) {
    AnalyticsSdk.instance.trackNavigation(
      pageKey: _navTabs[index].key,
      pageName: _navTabs[index].name,
    );
...
#### updateCurrentPage — Tab 切换时同步当前页面

适用于 `PageView` + `AutomaticKeepAliveClientMixin` 场景：Tab 切换不走路由生命周期，需手动告知 SDK 当前页面，保证后续 `page_click` 归属正确，且从详情页 pop 返回后能恢复正确的子页面 key。

```dart
// 预定义子 Tab（建议用 AppPageEnum，也可自定义字符串）
const _subPages = [
  AppPageEnum.videoList,
  AppPageEnum.novelList,
  AppPageEnum.comicList,
];
...
PageView(
  onPageChanged: (index) {
    ...
    // 同时手动上报 app_page_view（PageView 不走路由观察者）
    AnalyticsSdk.instance.track(AppPageViewEvent(
      pageKey: page.key,
      pageName: page.name,
      referrerPageKey: _previousPageKey,
      referrerPageName: _previousPageName,
    ));
  },
)
```
...
AnalyticsSdk.instance.track(AdImpressionEvent(
  pageKey: AppPageEnum.home.key,
  pageName: AppPageEnum.home.name,
...
预定义页面路由常量，所有需要传 `page_key` / `page_name` 的地方均可使用，也可直接传自定义字符串。

```dart
// 用法示例
AnalyticsSdk.instance.trackNavigation(
  pageKey: AppPageEnum.videoDetail.key,
  pageName: AppPageEnum.videoDetail.name,
);
```
...
| 枚举值 | key | name |
|--------|-----|------|
| `AppPageEnum.launch` | `launch` | 启动页 |
| `AppPageEnum.guide` | `guide` | 引导页 |
| `AppPageEnum.home` | `home` | 首页 |
| `AppPageEnum.mainPage` | `main_page` | 主页 |
| `AppPageEnum.discover` | `discover` | 发现页 |
| `AppPageEnum.search` | `search` | 搜索页 |
| `AppPageEnum.searchResult` | `search_result` | 搜索结果页 |
| `AppPageEnum.appCenter` | `app_center` | 应用中心 |
| `AppPageEnum.community` | `community` | 社区页 |
| `AppPageEnum.square` | `square` | 广场页 |
| `AppPageEnum.mine` | `mine` | 我的 |
| `AppPageEnum.userCenter` | `user_center` | 个人中心 |
| `AppPageEnum.userHome` | `user_home` | 用户主页 |
| `AppPageEnum.authorHome` | `author_home` | 作者主页 |
| `AppPageEnum.login` | `login` | 登录页 |
| `AppPageEnum.register` | `register` | 注册页 |
| `AppPageEnum.messageCenter` | `message_center` | 消息中心 |
| `AppPageEnum.setting` | `setting` | 设置页 |
| `AppPageEnum.recharge` | `recharge` | 充值页 |
| `AppPageEnum.rechargeRecord` | `recharge_record` | 充值记录页 |
| `AppPageEnum.mineCollect` | `mine_collect` | 我的收藏 |
| `AppPageEnum.mineHistory` | `mine_history` | 我的历史 |
| `AppPageEnum.mineVip` | `mine_vip` | 会员中心页 |

#### 视频类

| 枚举值 | key | name |
|--------|-----|------|
| `AppPageEnum.videoList` | `video_list` | 视频列表页 |
| `AppPageEnum.videoCategory` | `video_category` | 视频分类页 |
| `AppPageEnum.videoRank` | `video_rank` | 视频榜单页 |
| `AppPageEnum.videoSearch` | `video_search` | 视频搜索页 |
| `AppPageEnum.videoDetail` | `video_detail` | 视频详情页 |
| `AppPageEnum.videoPlay` | `video_play` | 视频播放页 |

#### 小说类

| 枚举值 | key | name |
|--------|-----|------|
| `AppPageEnum.novelList` | `novel_list` | 小说列表页 |
| `AppPageEnum.novelCategory` | `novel_category` | 小说分类页 |
| `AppPageEnum.novelRank` | `novel_rank` | 小说榜单页 |
| `AppPageEnum.novelDetail` | `novel_detail` | 小说详情页 |
| `AppPageEnum.novelRead` | `novel_read` | 小说阅读页 |
| `AppPageEnum.novelChapterList` | `novel_chapter_list` | 小说章节页 |

#### 漫画类

| 枚举值 | key | name |
|--------|-----|------|
| `AppPageEnum.comicList` | `comic_list` | 漫画列表页 |
| `AppPageEnum.comicCategory` | `comic_category` | 漫画分类页 |
| `AppPageEnum.comicRank` | `comic_rank` | 漫画榜单页 |
| `AppPageEnum.comicDetail` | `comic_detail` | 漫画详情页 |
| `AppPageEnum.comicRead` | `comic_read` | 漫画阅读页 |
| `AppPageEnum.comicChapterList` | `comic_chapter_list` | 漫画章节页 |

> 不在列表中的页面直接传字符串即可，无需扩展枚举：
> ```dart
> AnalyticsSdk.instance.trackNavigation(pageKey: 'live_room', pageName: '直播间');
> ```
...
| `sdk_version` | `String` | SDK 版本号（自动写入，当前为 `1.3.0`） |
