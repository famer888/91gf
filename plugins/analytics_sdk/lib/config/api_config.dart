...
class ApiConfig {
  ...
  static String buildUrl(String baseDomain, String path, {String? appId}) {
    ...
    // 保留 baseDomain 中的路径前缀（如 /v2），仅在 host 成功解析时使用
    // 去掉末尾 / 避免与 normalizedPath 拼出双斜杠
    final basePath = uri.host.isNotEmpty && uri.path.isNotEmpty && uri.path != '/'
        ? (uri.path.endsWith('/')
    ...
    // 构建完整 URL
    String url = '$scheme://$host$portSuffix$basePath$normalizedPath';
