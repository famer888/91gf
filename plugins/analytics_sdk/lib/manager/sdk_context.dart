...
/// SDK 全局上下文：持有所有公共字段、提供 event_id 计算。
///
/// 作为 SDK 内部的单一状态来源（SSOT），所有事件构造时从此处读取公共字段。
/// 不对外暴露，接入方无需直接使用。
class SdkContext {
  ...
  /// SID 提供者（由 AnalyticsSdk.init() 注入）。
  /// 调用时同时返回当前 SID 并记录用户行为，避免 entity 层直接依赖 SessionManager。
  static String Function()? sidProvider;
  ...
  static Map<String, dynamic> generateCommonFields(String event) {
    ...
    String sid;
    try {
      sid = sidProvider?.call() ?? 'fallback_${now.millisecondsSinceEpoch}';
    } catch (_) {
