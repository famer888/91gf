    ...
    return _uploadLock.synchronized(() async {
      ...
      try {
        ...
        try {
          ...
          // 发送前持久化 in-flight 标记，崩溃重启后将过滤这批事件（at-most-once）
          await _persistence.markInFlight(batch);
          ...
          final response = await _client
              ...
              .timeout(SdkConfig.totalRequestTimeout);
          ...
          // 无论成功、失败还是异常，确保 in-flight 文件被清除（幂等）
          try {
            await _persistence.clearInFlight();
          } catch (_) {}
        }
      } finally {
        _isUploading = false;
