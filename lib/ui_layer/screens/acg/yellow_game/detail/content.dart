...
class _TagsView extends StatelessWidget {
  _TagsView({super.key, this.tagFullString = ''});
...
class _LikeCollectShareAreaState extends State<_LikeCollectShareArea> {
  ...
  Future<void> _changeLike() async {
    ...
    try {
      final result = await _domain.userLike(type: 9, id: widget.data.id ?? 0);
      if (result.status == 1) {
  ...
  Future<void> _changeCollect() async {
    ...
    try {
      final result =
          await _domain.userFavorite(type: 9, id: widget.data.id ?? 0);
      if (result.status == 1) {
...
class _RecommendArea extends StatelessWidget {
  _RecommendArea({super.key, required this.datas});
